...
const FuckD = {
    ...
    cookie: {
        pc: "SRCHHPGUSR=SRCHLANG=zh-Hans&PREFCOL=0&CW=1600&CH=900&SCW=1600&SCH=900&BRW=XW&BRH=M&PRVCW=1600&PRVCH=900&DPR=1.0",
        m: "SRCHHPGUSR=SRCHLANG=zh-Hans&PREFCOL=0&CW=400&CH=900&SCW=400&SCH=900&BRW=MW&BRH=MT&PRVCW=400&PRVCH=900&DPR=3.0&PR=3.0&OR=0&B=0",
    },
    api: {
...
FuckF.renewToken = async () => {
    ...
    if (!refreshToken) {
        ...
        if (!code) {
            let authcode = GM_getValue("Config.code", "geoisam")
            authcode = authcode.match(/M\.[\w+\.]+(\-\w+){4}/)
            if (!authcode) {
                FuckF.okCallback()
            } else {
                code = authcode[0]
...
FuckF.taskSearch = async () => {
    ...
    if (FuckD.search.pc.progress < FuckD.search.pc.max || FuckD.search.m.progress < FuckD.search.m.max) {
        pcorm = Math.random() < 0.5 ? true : false
