...
const FuckF = {
    ...
    getDatetime(num = true) {
        ...
        const dateChinaSlash = dateChina.replaceAll("-", "/")
    ...
    try {
        ...
        if (result && FuckF.isJSON(result)) {
            ...
            if (refreshToken && accessToken) {
                GM_setValue("Config.token", refreshToken)
                FuckD.bing.token = accessToken
                return true
            }
        }
        return false
...
FuckF.taskSign = async () => {
    ...
    if (FuckD.sign.point >= 0) {
        ...
        if (FuckD.sign.date != FuckD.bing.dateNowNum) {
            ...
            if (!FuckD.bing.initCheck) FuckF.log("\ud83d\udfe3", FuckD.bing.fraudTips, true)
        }
...
FuckF.taskSearch = async () => {
    ...
    if (FuckD.search.pc.progress >= FuckD.search.pc.max && FuckD.search.m.progress >= FuckD.search.m.max) {
        ...
        const url = `https://bing.com/rewardsapp/ncheader?ver=${FuckF.getTimestamp(5)}&IID=SERP.5047&IG=${FuckF.getRandomUUID(true)}&ajaxreq=1`
...
return new Promise((resolve, reject) => {
    if (!FuckD.bing.fraudTips) reject()
    FuckD.search.limit = FuckF.getScopeRandomNum(3, 7)
    FuckD.bing.dateNowNum = Number(FuckF.getDatetime())
    FuckD.bing.initCheck = ["geoi", "sam"].every(str => GM_info.script.header.includes(str))
    if (!FuckD.bing.initCheck) FuckF.log("\ud83d\udfe3", FuckD.bing.fraudTips, true)
    ...
    if (!tasksArr) {
        ...
        FuckD.sign.date = tasksArr.sign
        FuckD.read.date = tasksArr.read
        FuckD.promos.date = tasksArr.promos
        FuckD.search.date = tasksArr.search
    }
    if (FuckD.api.mode != "offline") {
    ...
    FuckF.signStart = async () => {
        try {
            ...
            result ? FuckF.tasksEnd() : setTimeout(() => { FuckF.signStart() }, FuckD.bing.time)
        } catch (e) {
            FuckD.sign.end++
            FuckF.tasksEnd()
        }
    ...
    FuckF.readStart = async () => {
        try {
            ...
            result ? FuckF.tasksEnd() : setTimeout(() => { FuckF.readStart() }, FuckD.bing.time)
        } catch (e) {
            FuckD.read.end++
            FuckF.tasksEnd()
        }
    ...
    FuckF.promosStart = async () => {
        try {
            ...
            result ? FuckF.tasksEnd() : setTimeout(() => { FuckF.promosStart() }, FuckD.bing.time)
        } catch (e) {
            FuckD.promos.end++
            FuckF.tasksEnd()
        }
    ...
    FuckF.searchStart = async () => {
        try {
            ...
            const timespan = FuckF.getScopeRandomNum(10000, 30000)
            result ? FuckF.tasksEnd() : setTimeout(() => { FuckF.searchStart() }, timespan)
        } catch (e) {
            FuckD.search.end++
            FuckF.tasksEnd()
        }
