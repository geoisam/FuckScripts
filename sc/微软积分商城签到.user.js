...
const FuckD = {
    ...
    ua: {
        pc: "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36 Edg/123.0.2420.81 BingSapphire/31.4.2110003555",
        m: "Mozilla/5.0 (Linux; Android 16; MCE16 Build/BP3A.250905.014; ) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/123.0.0.0 Mobile Safari/537.36 EdgA/123.0.2420.102 BingSapphire/31.4.2110003555",
    },
    cookie: {
        pc: "SRCHHPGUSR=SRCHLANG=zh&PREFCOL=0&CW=1600&CH=900&SCW=1600&SCH=900&BRW=XW&BRH=M&PRVCW=1600&PRVCH=900&DPR=1.0",
        m: "SRCHHPGUSR=SRCHLANG=zh&PREFCOL=0&CW=400&CH=900&SCW=400&SCH=900&BRW=MW&BRH=MT&PRVCW=400&PRVCH=900&DPR=3.0&PR=3.0&OR=0&B=0",
    },
    api: {
...
FuckF.taskSearch = async () => {
    ...
    if (pcorm) {
        query = query + `&${regionMKT ? "mkt=zh-CN" : ""}&FORM=ANWSB4&PC=esb004`
        headers = {
            "content-type": "application/x-www-form-urlencoded; charset=utf-8",
            "user-agent": FuckD.ua.pc,
            "cookie": `${FuckD.cookie.pc}; SRCHUSR=DOB=${FuckD.bing.dateNowNum}&DS=1; ${cookieMKT}; SRCHD=AF=NOFORM`,
            "x-requested-with": "www.microsoft.com",
            "dnt": 1,
        }
    } else {
        query = query + `&${regionMKT ? "setmkt=zh-CN" : ""}&PC=EMMX01&form=LWS003`
        headers = {
            "content-type": "application/x-www-form-urlencoded; charset=utf-8",
            "user-agent": FuckD.ua.m,
