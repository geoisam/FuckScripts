...
FuckF.taskSign = async () => {
    if (!FuckD.tasks.sign || FuckD.sign.date == FuckD.bing.dateNowNum || FuckD.sign.times > 2) {
        FuckD.sign.end++
        return true
    }
...
FuckF.taskRead = async () => {
    if (!FuckD.tasks.read || FuckD.read.times > 2) {
        FuckD.read.end++
        return true
    }
    const fucker = await FuckF.mainlandCheck()
...
FuckF.taskPromos = async () => {
    if (!FuckD.tasks.promos || FuckD.promos.times > 2) {
        FuckD.promos.end++
        return true
    }
    const fucker = await FuckF.mainlandCheck()
