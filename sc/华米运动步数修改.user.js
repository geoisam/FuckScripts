// ==UserScript==
// @name         华米运动步数修改
// @namespace    https://geoisam.github.io
// @version      2.2.0
// @description  每天自动修改并同步 微信运动/支付宝运动 步数，需登录 Zepp/Zepp Life 绑定第三方数据，支持多账号处理和消息推送
// @author       geoisam@qq.com
// @icon         data:image/png;base64,
// @homepage     https://github.com/geoisam/FuckScripts
// @supportURL   https://github.com/geoisam/FuckScripts/issues
// @crontab      * * once * *
// @connect      api-user.huami.com
// @connect      account.huami.com
// @connect      account-cn.huami.com
// @connect      api-mifit-cn.huami.com
// @connect      qyapi.weixin.qq.com
// @connect      oapi.dingtalk.com
// @connect      open.feishu.cn
// @connect      push.i-i.me
// @grant        unsafeWindow
// @grant        GM_xmlhttpRequest
// @grant        GM_notification
// @grant        GM_openInTab
// @grant        GM_getValue
// @grant        GM_setValue
// @grant        GM_info
// @grant        GM_log
// @tips         此脚本一直为 开源免费 使用，如果你是从某些地方买的话，你就是被骗了
// ==/UserScript==


/* ==UserConfig==
Config:
    data:
        title: 账号#密码（一行一组）
        type: textarea
        description: xxxxx@xx.xxx#xxxxxxxx
    min:
        title: 步数最小值（MIN）
        type: number
        default: 17760
        min: 1
        max: 100000
        description: 17760
    max:
        title: 步数最大值（MAX）
        type: number
        default: 82240
        min: 1
        max: 100000
        description: 82240
    span:
        title: 运行间隔（至少10秒）
        type: number
        default: 10
        min: 10
        description: 10
        unit: 秒
Notice:
    bro:
        title: 浏览器通知（当前脚本）
        type: checkbox
        default: true
    wework:
        title: 企业微信消息推送（群机器人）
        type: text
        password: true
        description: xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx
    dingding:
        title: 钉钉群机器人（不加签，关键词：#）
        type: text
        password: true
        description: xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    feishu:
        title: 飞书群机器人（不加签，关键词：#）
        type: text
        password: true
        description: xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx
    pushme:
        title: PushMe（push.i-i.me）
        type: text
        password: true
        description: xxxxxxxxxxxxxxxxxxxx
==/UserConfig== */


const FuckD = {
    wh: [
        {
            name: "企业微信",
            url: "https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=",
            key: GM_getValue("Notice.wework", false),
            msg: {
                "msgtype": "markdown_v2",
                "markdown_v2": {
                    get content() {
                        return `> ${FuckD.hm.datetimeChina}\n\n ## ${GM_info.script.name}\n ${FuckD.hm.sendMSG}`
                    }
                },
            },
            docs: "https://developer.work.weixin.qq.com/document/path/91770"
        },
        {
            name: "钉钉",
            url: "https://oapi.dingtalk.com/robot/send?access_token=",
            key: GM_getValue("Notice.dingding", false),
            msg: {
                "msgtype": "markdown",
                "markdown": {
                    "title": GM_info.script.name,
                    get text() {
                        return `> ${FuckD.hm.datetimeChina}\n ### ${GM_info.script.name}\n ${FuckD.hm.sendMSG}`
                    }
                },
            },
            docs: "https://open.dingtalk.com/document/orgapp/custom-robots-send-group-messages"
        },
        {
            name: "飞书",
            url: "https://open.feishu.cn/open-apis/bot/v2/hook/",
            key: GM_getValue("Notice.feishu", false),
            msg: {
                "msg_type": "interactive",
                "card": {
                    "schema": "2.0",
                    "header": {
                        "title": {
                            "tag": "plain_text",
                            "content": GM_info.script.name
                        },
                        "template": "orange"
                    },
                    "body": {
                        "elements": [{
                            "tag": "markdown",
                            "text_align": "center",
                            get content() {
                                return `#### ${FuckD.hm.datetimeChina}\n ${FuckD.hm.sendMSG}`
                            }
                        }]
                    }
                }
            },
            docs: "https://open.feishu.cn/document/client-docs/bot-v3/add-custom-bot"
        },
        {
            name: "PushMe",
            url: "https://push.i-i.me/?push_key=",
            key: GM_getValue("Notice.pushme", false),
            msg: {
                "type": "markdown",
                "title": GM_info.script.name,
                get content() {
                    return `\n ${FuckD.hm.sendMSG}`
                }
            },
            docs: "https://push.i-i.me/docs/index"
        },
    ],
    hm: {
        fraudTips: "\u6e29\u99a8\u63d0\u793a\uff1a\u60a8\u53ef\u80fd\u662f\u76d7\u7248\u811a\u672c\u7684\u53d7\u5bb3\u8005\uff0c\u8bf7\u6ce8\u610f\u9632\u8303\u8bc8\u9a97\uff01",
        loginUrl: "https://user.huami.com/universalLogin/#/login?project_redirect_uri=https://www.huami.com/",
        deviceID: "88CC5224060006C4",
        key: "xeNtBVqzDc6tuNTh",
        iv: "MAAAYAAAAAAAAABg",
        thirdName: "huami",
        tokenData: GM_getValue("Config.token", []),
        headers: {
            "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
            "user-agent": "MiFit/6.12.0 (MCE16; Android 16; Density/1.5)",
            "app_name": "com.xiaomi.hm.health",
        },
    }
}

const FuckF = {
    getRandomNum(min, max) {
        return Math.floor(Math.random() * (max + 1 - min) + min)
    },

    getTimestamp(sec = false) {
        const timeNow = Date.now()
        return sec ? Math.floor(timeNow / 1000) : timeNow
    },

    getDatetime(only = false) {
        const timeUTC = new Date()
        const newDateChina = new Date(timeUTC.setUTCHours(timeUTC.getUTCHours() + 8))
        const dateChina = newDateChina.toISOString().split("T")[0]
        const datetimeChina = newDateChina.toISOString().split(".")[0].replace("T", " ")
        return only ? dateChina : datetimeChina
    },

    isJSON(s) {
        try {
            const j = JSON.parse(s)
            const p = Array.isArray(j) || (typeof j === 'object' && j !== null)
            if (p) return true
            return false
        } catch (e) {
            return false
        }
    },

    formatStr(str) {
        const lines = str.split(/\r?\n/)
        const result = []
        for (const line of lines) {
            if (line.trim() === "") continue
            const pairs = line.split(",")
            for (const pair of pairs) {
                const trimmedPair = pair.trim()
                if (trimmedPair === "") continue
                const parts = trimmedPair.split("#")
                if (
                    parts.length !== 2 ||
                    parts[0].trim() === "" ||
                    parts[1].trim() === ""
                ) {
                    return []
                }
                result.push([parts[0].trim(), parts[1].trim()])
            }
        }
        return result
    },

    mergeArray(group1, group2) {
        const group1Map = group1.reduce((map, [user, pwd]) => {
            map[user] = pwd
            return map
        }, {})
        const group2Map = group2.reduce((map, [user, token]) => {
            map[user] = token
            return map
        }, {})
        const accounts = [...new Set(group1.map(item => item[0]))]
        const mergedArray = accounts.map(user => [
            user,
            group1Map[user],
            group2Map[user] || ""
        ])
        return mergedArray
    },

    updateArray(group1, group2) {
        const map = new Map()
        group1.forEach(item => {
            map.set(item[0], [item[1], item[2]])
        })
        group2.forEach(item => {
            const [user, token, time] = item
            map.set(user, [token, time])
        })
        return Array.from(map, ([user, [token, time]]) => [user, token, time])
    },

    log(title, text, push = false) {
        GM_log(title + text + "🔚")
        if (!GM_getValue("Notice.bro", true) || !push) return
        GM_notification({
            text: text,
            title: GM_info.script.name + title,
            onclick: () => {
                GM_openInTab("https://github.com/geoisam", { active: true, insert: true, setParent: true })
            },
        })
    },

    xhr(options) {
        return new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
                ...options,
                cookies: false,
                timeout: 5000,
                ontimeout() {
                    this.log("🔴", `请求超时！`)
                    reject()
                },
                onload(xhr) {
                    if (xhr.status == 200) {
                        resolve(xhr.responseText)
                    } else if (xhr.status == 429) {
                        this.log("🟡", `请求过于频繁，请稍后再试！ 🔛${xhr.responseText}`, true)
                        reject()
                    } else {
                        this.log("🔴", `请求失败，状态码：${xhr.status} 🔛${xhr.responseText}`)
                        reject()
                    }
                },
                onerror(err) {
                    this.log("🔴", `请求发生异常！ 🔛${err}`)
                    reject()
                },
            })
        })
    },
}

return new Promise((resolve, reject) => {
    if (!FuckD.hm.fraudTips) reject()
    const userData = FuckF.formatStr(GM_getValue("Config.data", "#"))
    if (userData.length == 0) FuckF.log("🔴", "账号#密码填写格式错误，请重新输入！", true), reject()
    const accounts = FuckF.mergeArray(userData, FuckD.hm.tokenData)
    const initCheck = ["geoi", "sam"].every(str => GM_info.script.header.includes(str))
    if (!initCheck) FuckF.log("🟣", FuckD.hm.fraudTips, true)
    const stepsMIN = GM_getValue("Config.min", 17760)
    const stepsMAX = GM_getValue("Config.max", 82240)
    const timeDelay = GM_getValue("Config.span", 10)

    FuckF.tasksStart = async () => {
        for (const [index, account] of accounts.entries()) {
            const total = accounts.length
            FuckF.log("🔵", `开始处理第 ${index + 1}/${total} 个账号「${account[0]}」`)
            let tokenInfo
            account[3] = 0
            if (!initCheck) FuckF.log("🟣", FuckD.hm.fraudTips, true)
            if (account[0].indexOf("@") !== -1) {
                FuckD.hm.userName = account[0]
            } else {
                FuckD.hm.userName = `+86${account[0]}`
                FuckD.hm.thirdName = "huami_phone"
            }
            try {
                if (!account[2]) {
                    tokenInfo = await FuckF.login(account)
                } else {
                    tokenInfo = await FuckF.renew(account)
                    if (!tokenInfo) tokenInfo = await FuckF.login(account)
                }
                if (tokenInfo) {
                    FuckD.hm.tokenData = FuckF.updateArray(FuckD.hm.tokenData, [[account[0], FuckD.hm.tokenInfo.login, FuckF.getTimestamp()]])
                    GM_setValue("Config.token", FuckD.hm.tokenData)
                    FuckF.log("🟣", `「${account[0]}」数据 JSON 存储更新完成！`)
                    await FuckF.submit(account)
                }
                account[3] = FuckD.hm.todaySteps
            } catch (e) {
                FuckF.log("🔴", `「${account[0]}」处理出错 🔛${e}`)
            }
            if (index < total - 1) {
                FuckF.log("🔵", `「${account[0]}」完成，等待 ${timeDelay} 秒后继续...`)
                await new Promise(resolve => setTimeout(resolve, (timeDelay - 3) * 1000))
            }
            await new Promise(resolve => setTimeout(resolve, 3000))
        }
        const dataCheck = FuckD.hm.tokenData
        const sevenDaysAgo = FuckF.getTimestamp() - 7 * 24 * 60 * 60 * 1000
        FuckD.hm.JSON = FuckD.hm.tokenData.filter(item => (item[2] >= sevenDaysAgo))
        if (dataCheck.length > 0 && FuckD.hm.tokenData != dataCheck) {
            GM_setValue("Config.token", FuckD.hm.tokenData)
            FuckF.log("🟣", "JSON 存储过期数据清理完成！")
        }
        let dataLOG = accounts.map(account => `|${account[0]}|${account[3]}|\n`).join("")
        dataLOG = dataLOG ? `|账号|步数|\n|:---:|:---:|\n${dataLOG}` : "**ERROR**\n"
        if (!initCheck) dataLOG = `> ${FuckD.hm.fraudTips}\n\n \n${dataLOG}`
        FuckD.hm.sendMSG = `---\n ${dataLOG}\n ---\n **[Version ${GM_info.script.version}](https://github.com/geoisam)**\n`
        await FuckF.send(FuckD.wh)
        FuckF.log("🟣", "所有任务处理完成，具体请查看日志！", true)
        if (!initCheck) FuckF.log("🟣", FuckD.hm.fraudTips, true)
        resolve()
    }

    FuckF.login = async (account) => {
        let message = `「${account[0]}」获取 AccessToken `
        try {
            const result = await FuckF.xhr({
                method: "POST",
                url: `https://api-user.huami.com/registrations/${FuckD.hm.userName}/tokens`,
                headers: FuckD.hm.headers,
                data: new URLSearchParams({
                    "client_id": "HuaMi",
                    "country_code": "CN",
                    "json_response": true,
                    "name": FuckD.hm.userName,
                    "password": account[1],
                    "redirect_uri": "https://s3-us-west-2.amazonaws.com/hm-registration/successsignin.html",
                    "state": "REDIRECTION",
                    "token": "access",
                }).toString(),
            })
            if (result && FuckF.isJSON(result)) {
                const res = JSON.parse(result)
                if (res.access) {
                    FuckD.hm.tokenTemp = res.access
                    FuckF.log("🟢", `${message}成功！`)
                } else {
                    FuckF.log("🟡", `「${account[0]}」用户名或密码错误！🔛${result}`, true)
                    return false
                }
            } else {
                FuckF.log("🔴", `${message}失败！🔛${result}`)
                return false
            }
        } catch (e) {
            return null
        }

        message = `「${account[0]}」获取 UserInfo `
        try {
            const result = await FuckF.xhr({
                method: "POST",
                url: "https://account.huami.com/v2/client/login",
                headers: FuckD.hm.headers,
                data: new URLSearchParams({
                    "app_name": "com.xiaomi.hm.health",
                    "country_code": "CN",
                    "code": FuckD.hm.tokenTemp,
                    "device_id": "02:00:00:00:00:00",
                    "device_model": "android_phone",
                    "app_version": "6.12.0",
                    "grant_type": "access_token",
                    "allow_registration": false,
                    "source": "com.xiaomi.hm.health",
                    "third_name": FuckD.hm.thirdName,
                }).toString(),
            })
            if (result && FuckF.isJSON(result)) {
                const res = JSON.parse(result)
                if (res.token_info) {
                    FuckD.hm.tokenInfo = {
                        id: res.token_info.user_id,
                        app: res.token_info.app_token,
                        login: res.token_info.login_token
                    }
                    FuckF.log("🟢", `${message}成功！`)
                    return true
                }
            }
            FuckF.log("🔴", `${message}失败！🔛${result}`)
            return false
        } catch (e) {
            return null
        }
    }

    FuckF.renew = async (account) => {
        let message = `「${account[0]}」获取 LoginToken `
        try {
            const result = await FuckF.xhr({
                url: `https://account-cn.huami.com/v1/client/renew_login_token?login_token=${account[2]}`,
                headers: FuckD.hm.headers,
            })
            if (result && FuckF.isJSON(result)) {
                const res = JSON.parse(result)
                if (res.token_info) {
                    FuckD.hm.tokenTemp = res.token_info.login_token
                    FuckF.log("🟢", `${message}成功！`)
                } else {
                    FuckF.log("🟡", `${message}失败！ 🔛${result}`)
                    return false
                }
            } else {
                FuckF.log("🔴", `${message}失败！🔛${result}`)
                return false
            }
        } catch (e) {
            return null
        }

        message = `「${account[0]}」获取 AppToken `
        try {
            const result = await FuckF.xhr({
                url: `https://account-cn.huami.com/v1/client/app_tokens?login_token=${FuckD.hm.tokenTemp}`,
                headers: FuckD.hm.headers,
            })
            if (result && FuckF.isJSON(result)) {
                const res = JSON.parse(result)
                if (res.token_info) {
                    FuckD.hm.tokenInfo = {
                        id: res.token_info.user_id,
                        app: res.token_info.app_token,
                        login: FuckD.hm.tokenTemp,
                    }
                    FuckF.log("🟢", `${message}成功！`)
                    return true
                }
            }
            FuckF.log("🔴", `${message}失败！🔛${result}`)
            return false
        } catch (e) {
            return null
        }
    }

    FuckF.submit = async (account) => {
        FuckD.hm.todaySteps = 0
        const headers = { ...FuckD.hm.headers }
        headers.apptoken = FuckD.hm.tokenInfo.app
        const todaySteps = FuckF.getRandomNum(stepsMIN, stepsMAX)
        const dataJSON = {
            "date": FuckF.getDatetime(true),
            "data_hr": "/v7+".repeat(480),
            "data": [{
                "start": 0,
                "stop": 1439,
                "value": "AU".repeat(1440 * 2),
                "tz": 32,
                "did": FuckD.hm.deviceID,
                "src": 24,
            }],
            "summary": JSON.stringify({
                "v": 6,
                "stp": {
                    "ttl": todaySteps,
                    "dis": Math.floor(todaySteps * 0.7),
                    "cal": Math.floor(todaySteps / 25),
                    "wk": Math.floor(todaySteps / 120),
                },
                "goal": 8000,
            }),
            "source": 24,
            "type": 0,
        }

        let message = `「${account[0]}」步数数据提交`
        try {
            const result = await FuckF.xhr({
                method: "POST",
                url: `https://api-mifit-cn.huami.com/v1/data/band_data.json?t=${FuckF.getTimestamp()}`,
                headers: headers,
                data: new URLSearchParams({
                    "userid": FuckD.hm.tokenInfo.id,
                    "device_type": 0,
                    "last_source": 24,
                    "last_deviceid": FuckD.hm.deviceID,
                    "enableMultiDevice": 1,
                    "last_sync_data_time": FuckF.getTimestamp(true),
                    "data_json": JSON.stringify([dataJSON]),
                }).toString(),
            })
            if (result && FuckF.isJSON(result)) {
                const res = JSON.parse(result)
                if (res.code && res.code == 1) {
                    FuckD.hm.todaySteps = todaySteps
                    FuckF.log("🟣", `${message}完成！ 🔛${result}`)
                    return true
                }
            }
            FuckF.log("🔴", `${message}失败！🔛${result}`)
            return false
        } catch (e) {
            return null
        }
    }

    FuckF.send = async (webhook) => {
        FuckD.hm.datetimeChina = FuckF.getDatetime()
        await Promise.all(webhook.map(async (i) => {
            if (!i.key) return
            let message = `「${i.name}」消息推送`
            try {
                const result = await FuckF.xhr({
                    method: "POST",
                    url: i.url + i.key,
                    headers: {
                        "content-type": "application/json; charset=UTF-8",
                    },
                    data: JSON.stringify(i.msg),
                })
                if (result) FuckF.log("🟣", `${message}完成 🔛${result}`)
            } catch (e) {
                return null
            }
        }))
        return
    }

    FuckF.tasksStart()
})