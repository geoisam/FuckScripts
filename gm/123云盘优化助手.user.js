...
<div class="swal2-validation-message" id="swal2-validation-message" style="display: none;"></div>
        ...
        autoGoGoGo() {
            ...
            GM_addStyle(`
.advBanner,
.ant-modal-root:has(img[src*="tuia-fa"]),
[class*="banner_guide"],
[class*="adv-container"],
...
.webbody.svip-body,.app-body.svip-body,.backgroundImage,.verifyBox { background-image: none !important; }
`)
...
`)
            ...
            const fuckRules = [
                ...
                {
                    runat: "end",
                    match: (url) => ["/file/download_info", "/file/batch_download_info", "/share/download/info", "/file/batch_download_share_info", "/play/info", "/download/traffic", "/prepayment"].some(path => url.pathname.includes(path)),
                    action: (res, url) => {
                        if (pjs.getValue("popup_payment_hidden")) {
                            ...
                            if (res?.code === 5113 || res?.code === 5114 || res?.message?.includes("下载流量已超出") || res?.message?.includes("播放超限") || res?.message?.includes("免费流量不足") || res?.data?.tipDesc?.includes("本次下载超限")) {
