...
// @version      1.1.0
// @description  优化123云盘页面，支持电脑端和移动端
...
`)
            ...
            const fuckRules = [
                {
                    runat: "end",
                    match: (url) => ["/file/download_info", "/file/batch_download_info", "/share/download/info", "/file/batch_download_share_info", "/download/traffic", "/play/info"].some(path => url.pathname.includes(path)),
                    action: (res, url) => {
                        if (pjs.getValue("popup_payment_hidden")) {
                            if (url.pathname.includes("/download/traffic")) {
                                res.data.isBlocked = null
                                res.data.isTrafficExceeded = null
                            }
