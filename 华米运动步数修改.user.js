...
return new Promise((resolve, reject) => {
    ...
    const initCHECK = JSON.stringify(GM_info.script.header).includes(4285)
    ...
    if (!initCHECK) FuckF.pushMsg("🟣", FuckD.hm.fraudTIP, true)
    ...
    FuckF.tasksStart = async (accounts) => {
        for (const [index, account] of accounts.entries()) {
            ...
            try {
                ...
                if (!tokenInfo) {
                    ...
                    account[3] = account[3] ? todaySteps : 0
                }
            } catch (e) {
        ...
        if (!initCHECK) dataLOG = `> ${FuckD.hm.fraudTIP}\n\n \n${dataLOG}`
