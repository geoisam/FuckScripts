                ...
                if (xhr.status == 200) {
                    if (pjs.isJSONParsable(res)) {
                        ...
                        if (!res.error_code) {
                            ...
                            pjs.pushMsg("Token过期失效🟡", "已清除Token数据，请重新运行试试！")
                            resolve(false)
                        }
                    } else {
                        GM_log(`Code：${xhr.status}🔴\n\n${res}`)
...
return new Promise((resolve, reject) => {
    const isExpired = (GM_getValue("timestamp", 0) + pjs.oneDAY) < Date.now()
    ...
    pjs.tasksStart = async function () {
        try {
            if (!isExpired || GM_getValue("login_token", false)) {
                ...
                result ? pjs.submitStart(result.id, result.token) : (pjs.pushMsg("Token获取失败🔴", "请尝试手动运行或查看运行日志"), resolve())
            } else {
