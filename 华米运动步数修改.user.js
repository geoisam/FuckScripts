...
const pjs = {
    t: {
        ...
        homePAGE: "https://github.com/geoisam",
    },
    hm: {
...
pjs.pushMsg = function (title, text, push = false) {
    ...
    GM_notification({
        ...
        onclick: () => {
            GM_openInTab(pjs.t.homePAGE, { active: true, insert: true, setParent: true })
        }
    })
}
...
pjs.encryptData = function (data) {
    return new Promise((resolve, reject) => {
        pjs.encryptAES(data, pjs.hm.key, pjs.hm.iv).then((res) => { resolve(res) }).catch((e) => { reject(e) })
    })
}
    ...
    return new Promise((resolve, reject) => {
        GM_xmlhttpRequest({
            ...
            data: new URLSearchParams({
                "userid": info.id,
                "device_type": 0,
                "last_deviceid": pjs.hm.deviceID,
                "last_update_time": pjs.t.secondNOW,
...
return new Promise((resolve, reject) => {
    ...
    pjs.tasksStart = async (accounts) => {
        for (const [index, account] of accounts.entries()) {
            ...
            if (index < accounts.length - 1) {
                ...
                await new Promise(resolve => setTimeout(resolve, (pjs.t.timeDELAY - 1) * 1000))
            }
            await new Promise(resolve => setTimeout(resolve, 1000))
        }
        pjs.t.sevenDaysAgo = Date.now() - 7 * 24 * 60 * 60 * 1000
        GM_setValue("Config.token", GM_getValue("Config.token", []).filter(item => (item[2] >= pjs.t.sevenDaysAgo)))
        pjs.t.dataLOG = accounts.map(account => `|${account[0]}|${account[3]}|\n`).join("")
        pjs.t.sendMsg = `---\n |账号|步数|\n |:---:|:---:|\n${pjs.t.dataLOG}\n ---\n **[Version ${GM_info.script.version}](${pjs.t.homePAGE})**\n`
    ...
    pjs.getNewToken = async (account) => {
        try {
            ...
            const save = pjs.updateArray(GM_getValue("Config.token", []), [[account[0], token.login, Date.now()]])
    ...
    pjs.renewToken = async (account) => {
        try {
            ...
            const save = pjs.updateArray(GM_getValue("Config.token", []), [[account[0], login, Date.now()]])
            GM_setValue("Config.token", save)
