...
<label class="pjs-setting-label">隐藏未登录下载付费提示<input type="checkbox" id="S-Payment" ${pjs.getValue("nologin_payment_hidden") ? "checked" : ""} "><span class="pjs-setting-checkbox"></span></label>
                ...
                document.querySelector("#S-SVIP").addEventListener("change", (e) => {
                    const targetItem = pjs.option.find(item => item.name == "display_svip_block")
                    targetItem.value = e.currentTarget.checked
                })
...
`)
            ...
            if (pjs.getValue("download_limit_remove")) {
                        ...
                        action: (res, url) => {
                            ...
                            if (res?.code === 5113 || res?.code === 5114 || res?.message?.includes("下载流量已超出")) {
                                if (url.pathname.includes("batch_download")) {
                                    ...
                                    res = {
                                        code: 400,
                                        message: "您今日下载流量已超出限制！已为您拦截付费弹窗",
                                        data: null
                                    }
                                }
                            }
                ...
                unsafeWindow.fetch = async function (input, init = {}) {
                    ...
                    if (rules.some(rule => rule.match(url) && rule.runat === "header")) {
                    ...
                    if (rules.some(rule => rule.match(url) && rule.runat === "end")) {
                        try {
                            let response = await originalFetch.apply(this, arguments)
                            let responseText = await response.text()
                            let res = fuckUniversal("fetch", url, responseText, "end")
                    ...
                    if (rules.some(rule => rule.match(url) && rule.runat === "start")) {
                        try {
                            let res = fuckUniversal("fetch", url, null, "start")
                            return new Response(res, {
                                status: 200,
                                statusText: "OK",
                ...
                unsafeWindow.XMLHttpRequest.prototype.send = function (data) {
                    ...
                    if (rules.find(rule => rule.match(url) && rule.runat === "start")) {
                        try {
                ...
                function fuckUniversal(from, url, data, type) {
                    ...
                    let rule = rules.find(r => r.match(url) && r.runat === type)
                    if (rule) {
